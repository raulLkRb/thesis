...
#ifndef _axi_algorithm_dato_H_
#define _axi_algorithm_dato_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "axi_algorithm_dato_memcore.h"

SC_MODULE(axi_algorithm_dato) {
...
protected:
    ...
    // buffer instances
    axi_algorithm_dato_memcore* buffer[BufferCount];
...
public:
    ~axi_algorithm_dato();
    SC_CTOR(axi_algorithm_dato) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "axi_algorithm_dato_memcore_U_";
            name += szTmp;

            buffer[i] = new axi_algorithm_dato_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "axi_algorithm_dato.trace");
            // ports
