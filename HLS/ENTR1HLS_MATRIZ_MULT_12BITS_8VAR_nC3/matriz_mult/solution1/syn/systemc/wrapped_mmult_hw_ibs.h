...
#ifndef _wrapped_mmult_hw_ibs_H_
#define _wrapped_mmult_hw_ibs_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "wrapped_mmult_hw_ibs_memcore.h"

SC_MODULE(wrapped_mmult_hw_ibs) {
...
protected:
    ...
    wrapped_mmult_hw_ibs_memcore* wrapped_mmult_hw_ibs_memcore_U;
...
public:
    ...
    SC_CTOR(wrapped_mmult_hw_ibs) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        wrapped_mmult_hw_ibs_memcore_U = new wrapped_mmult_hw_ibs_memcore("wrapped_mmult_hw_ibs_memcore_U");
        wrapped_mmult_hw_ibs_memcore_U->clk      ( clk );
        wrapped_mmult_hw_ibs_memcore_U->reset    ( reset );
        wrapped_mmult_hw_ibs_memcore_U->ce0      ( i_ce0 );
        wrapped_mmult_hw_ibs_memcore_U->we0      ( i_we0 );
        wrapped_mmult_hw_ibs_memcore_U->address0 ( memcore_iaddr );
        wrapped_mmult_hw_ibs_memcore_U->d0       ( i_d0 );
        wrapped_mmult_hw_ibs_memcore_U->q0       ( i_q0 );

        wrapped_mmult_hw_ibs_memcore_U->ce1      ( t_ce0 );
        wrapped_mmult_hw_ibs_memcore_U->we1      ( t_we0 );
        wrapped_mmult_hw_ibs_memcore_U->address1 ( memcore_taddr );
        wrapped_mmult_hw_ibs_memcore_U->d1       ( t_d0 );
        wrapped_mmult_hw_ibs_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "wrapped_mmult_hw_ibs.trace");
            // ports
