...
#ifndef _wrapped_mmult_hw_hbi_H_
#define _wrapped_mmult_hw_hbi_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "wrapped_mmult_hw_hbi_memcore.h"

SC_MODULE(wrapped_mmult_hw_hbi) {
...
protected:
    ...
    // buffer instances
    wrapped_mmult_hw_hbi_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(wrapped_mmult_hw_hbi) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "wrapped_mmult_hw_hbi_memcore_U_";
            name += szTmp;

            buffer[i] = new wrapped_mmult_hw_hbi_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "wrapped_mmult_hw_hbi.trace");
            // ports
