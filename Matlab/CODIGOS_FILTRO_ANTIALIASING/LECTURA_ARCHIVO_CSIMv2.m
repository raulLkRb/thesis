...
myFolder = 'C:\Users\Raul\Documents\PROJECTS_2021\IP_CORES_D_2017\ALG_REDUCED_ARRAY_55_V17_DEBUG_for_55_tacteles\ALG\axi_algorithm\solution5\csim\report';
...
filePattern = fullfile(myFolder, '*log');
...
%     fclose(fid);
    ...
    fid=fopen(fullFileName, 'r+');
    full=textscan(fid, '%s', 'Delimiter', sprintf(';'));
    fclose(fid);
...
%     data = cellfun(@(x) strrep(x,',','.'), data, 'UniformOutput', false);
    ...
    a = cell2mat(cellfun(@str2num, full{1,1}, 'UniformOutput', false));
