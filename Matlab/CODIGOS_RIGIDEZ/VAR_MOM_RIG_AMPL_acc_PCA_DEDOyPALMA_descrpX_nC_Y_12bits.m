...
%             nIteraciones = size(descrpSupDedo{ii,1},1);
...
%                 nIteraciones = size(descrpSupDedo{ii,1},1);
        ...
        [MAT_AUXILIAR_OUT] = selectMomentInMATAUX(MAT_AUXILIAR,indMomentos,optDedoPalma,indDescrpCual);
        
        %% Aplicamos PCA al conjunto de datos para DEDO Y PALMA
        X = MAT_AUXILIAR_OUT(1:size(MAT_AUXILIAR_OUT,1)-1,:)';
        
        % Utilizamos la función [coeff,score,latent,tsquared,explained,mu] = pca(X); 
        % Entrada: 
        % X - matriz de observaciones, n x p (n, observaciones / p, variables)
        % Salidas: 
        % coeff - matriz de p x p, cada columna contiene los coeficientes
        % para una componentes principal. Las columnas están en orden decreciente
        % de variabilidad. 
        % score - matriz de representación de X en el espacio de las componentes
        % principales. Las filas de de score son las observaciones, y las columnas
        % los componentes.
        % latent - es un vector que contiene los autovalores de la matriz de
        % covarianza de X
        % tsquared, contiene al estadístico de Hotelling T^2 para cada punto. Como
        % no nos interesa para este problema, se puede poner como ~
        % explained - porcentaje de la variabilidad total explicada para cada
        % componente principal 
        % mu - media estimada de cada variable de la matriz X
        [coeff,score,latent,~,explained,mu] = pca(X);
        
        % Ejecutamos el comando anterior, y vemos lo que se obtiene en pc y en latent
        % La matriz de coeficientes pc, tiene tamaño 181 x 181, y el vector de
        % autovalores tiene tamaño 181 x 1. Normalizamos latent a valores de 0 a 1
        S=cumsum(latent)./sum(latent);
        
        % Miramos, por ejemplo, las 5 primeras posiciones de S. Nos damos cuenta
        % que con una componente principal se pueden explicar el 99.67% de los
        % datos. 
%             S(1:10)
        S
        
        % Si representamos los datos con 2 componentes principales, nos
        % podemos hacer una idea gráfica de por qué esto es así. Todos los puntos
        % que se obtienen están muy próximos a la componente 1. 
        % En realidad, sólo es necesaria una componente principal, pero para hacer
        % una idea gráfica he querido presentarlo con 2 gráficamente también
    %     biplot(coeff(:,1:2),'Scores',score(:,1:2));
        
        % Normalizamos los datos restando la media:
        % El siguiente código: restamos media
        Xn = [];
        for i = 1 : size(X,1)
            Xn(i,1:size(X,2)) = X(i,1:size(X,2))-mu;
        end

        %% Componentes principales, nC --> número de componentes principales.
        if(size(S,1)>6)
            nCArray = 1:6;
        else
            nCArray = 1:size(S,1);
...
%         nCIndice = 6;
        ...
        S_cell{indDescrpCual,1} = S;
    
        while(nCIndice <= size(nCArray,2))   
            ...
            %% Tomamos nC
            nC = nCArray(1,nCIndice);
            coeff_n=coeff(:,1:nC);
            % Para que los valores de los coeficientes estén en las columnas
            coeff_n=coeff_n';
            
            % Calculamos la matriz de dimensión reducida, que se corresponde con score:
            Xred=coeff_n*Xn';
...
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--MatConf--Momento',num2str(indMomentos));
...
%             strPrimera = strcat('DEDOyPALMA--PCA--12bits--Histogramadistancias--Momento',num2str(indMomentos));
%             strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%             strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
...
%             if(nCIndice == 1)
%                 [h_arriba] = figuresGaussiansPCA(nCIndice,nObjetos,centroide,dataTrainSWEEPFinal,labelTrainSWEEPFinal);
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol1--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
...
%                 [h_arriba] = figuresGaussiansPCA(nCIndice,nObjetos,centroide,dataTrainSWEEPFinal,labelTrainSWEEPFinal);
%                 % FIGURA 1
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol2dimC--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%                 close(figuraVol2C);
%                 % FIGURA 2
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol2dimB--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%                 close(figuraVol2B);
%                 % FIGURA 3
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol2dimA--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%                 [h_arriba] = figuresGaussiansPCA(nCIndice,nObjetos,centroide,dataTrainSWEEPFinal,labelTrainSWEEPFinal);
%                 % FIGURA 1
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol3dimB--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%                 close(figuraVol3B);
%                 % FIGURA 2
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol3dimA--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%             elseif(nCIndice == 4)
%                 [h_arriba] = figuresGaussiansPCA(nCIndice,nObjetos,centroide,dataTrainSWEEPFinal,labelTrainSWEEPFinal);
%                 strPrimera = strcat('DEDOyPALMA--PCA--12bits--figuraVol4--Momento',num2str(indMomentos));
%                 strSegunda = strcat(strPrimera,strcat('--nC',num2str(nCIndice)));
%                 strTercera = strcat(strSegunda,strcat('--Descriptor',descriptorName{indDescrpCual,1}));
%                 subtitle(strcat(num2str(pp_class),'%'));
%                 strCuarta = strcat('/figurasGaussianas/',strTercera);
...
%             end
    ...
    % Guardamos S_cell
    save(strcat('DEDOyPALMA--PCA--12bits--S_cell--Momento',num2str(indMomentos)),'S_cell');

    % Guardamos variables antes de seguir
    save(strcat('DEDOyPALMA--PCA--12bits--distMedCentr--Momento',num2str(indMomentos)),'distMedCentr');
    save(strcat('DEDOyPALMA--PCA--12bits--centroidesBuenosCell--Momento',num2str(indMomentos)),'centroidesBuenosCell');
    save(strcat('DEDOyPALMA--PCA--12bits--sigmaDesvTipicaClases--Momento',num2str(indMomentos)),'sigmaDesvTipicaClases');
    save(strcat('DEDOyPALMA--PCA--12bits--arrayPPClass--Momento',num2str(indMomentos)),'arrayPPClass');
